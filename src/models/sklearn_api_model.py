...
class Model(BaseEstimator, ClassifierMixin, RegressorMixin):
    def __init__(self, model, loss: Union[str, List] = 'log_loss', name='Model'):
        ...
        """
        self.best_estimator_: BaseEstimator = model
    ...
    def fit(self, X, y, optimization='skip', grid_params=None, fit_params={}, cv_folds=10):
        ...
        if optimization == 'grid':
            ...
            bayes_search = BayesSearchCV(
                self.best_estimator_, opt, scoring=self.get_scorer(), cv=cv_folds, refit=False)
            # # Check if the labels are 1D or 2D
            # if y.ndim > 1:
    ...
    def recursive_fit(self, X_train, y_train, eval_set, features, n_steps, early_stopping_rounds, **fit_params):
        ...
        num_boost_round = fit_params.get('n_estimators', 1000)
    ...
    def evaluate_recursive_predictions(self, model, X_val, y_val, features, n_steps):
        ...
        for uid in unique_ids:
            ...
            for index in range(len_id):
                ...
                for step in range(n_steps):
                    # Predict
                    pred = model.predict(
                        current_input.values.reshape(1, -1))[0]
                    preds.append(pred)

                    # Update current_input for next prediction
                    # Shift lag features
                    lag_features = [
                        col for col in current_input.index if 'y_lag' in col]
                    print(lag_features)
                    max_lag = max([int(col.split('_')[-1])
                    ...
                    if index + step + 1 < len(id_data):
                        ...
                        weight.append(
                            id_target['weight'].iloc[index + step + 1])
                    else:
        ...
        score = self.score_with_prediction(
            forecasts, y_vals, sample_weight=y_weight)
    ...
    def shapley_additive_explanation(self, df_set, outname, dir_output, mode='bar', figsize=(50, 25), samples=None, samples_name=None):
        ...
        try:
            ...
            shap_values: shap.Explanation = explainer(df_set, check_additivity=False)
            # plt.figure(figsize=figsize)
            fig, ax = plt.subplot(figsize=figsize)
            if mode == 'bar':
                shap.plots.bar(shap_values, show=False, max_display=40, ax=ax )
            elif mode == 'beeswarm':
                shap.plots.beeswarm(shap_values, show=False, max_display=40, ax=ax )
            else:
                raise ValueError(f'Unknow {mode} mode')
            
            shap_values_abs = np.abs(shap_values.values).mean(
                axis=0)  # Importance moyenne absolue des SHAP values
            # Indices des 10 plus importantes
            top_features_indices = np.argsort(shap_values_abs)[-10:]
            # Noms des 10 features
            self.top_features_ = df_set.columns[top_features_indices].tolist()

            # plt.tight_layout()
            plt.savefig(
                dir_output / f'{outname}_shapley_additive_explanation.png')
            plt.close('all')
            if samples is not None and samples_name is not None:
                ...
                for i, sample in enumerate(samples):
                    ...
                    shap.plots.force(
                        shap_values[sample], show=False, matplotlib=True, text_rotation=45, figsize=(30, 15))
                    plt.tight_layout()
                    plt.savefig(
                        dir_output / 'sample' / f'{outname}_{samples_name[i]}_shapley_additive_explanation.png')
                    plt.close('all')
...
class ModelFusion(Model):
    ...
    def plot_features_importance(self, X, y_list, y, names, outname, dir_output, mode='bar', figsize=(50, 25), deep=True):
        ...
        super().plot_features_importance(stacked_predictions, y,
                                         outname, dir_output, mode=mode, figsize=figsize)
...
class ModelVoting(Model):
    ...
    def fit(self, X_list, y_list, optimization='skip', grid_params_list=None, fit_params_list=None, cv_folds=10):
        ...
        if len(self.best_estimator_) != len(X_list) or len(X_list) != len(y_list):
            raise ValueError(
                "The length of models, X_list, and y_list must be the same.")
        ...
        for i, estimator in enumerate(self.best_estimator_):
            ...
            if optimization == 'grid':
                assert grid_params is not None
                grid_search = GridSearchCV(
                    estimator, grid_params, scoring=self.get_scorer(), cv=cv_folds, refit=False)
                grid_search.fit(X, y, **fit_params)
                best_params = grid_search.best_params_
                ...
                raise NotImplementedError(
                    "Bayesian optimization is not implemented in ModelVoting.")
            elif optimization == 'skip':
    ...
    def predict(self, X_list):
        ...
        if len(self.best_estimator_) != len(X_list):
            raise ValueError(
                "The length of models and X_list must be the same.")
    ...
    def predict_proba(self, X_list):
        ...
        for i, estimator in enumerate(self.best_estimator_):
            ...
            if hasattr(estimator, "predict_proba"):
                ...
                raise AttributeError(
                    f"The model at index {i} does not support predict_proba.")
    ...
    def aggregate_predictions(self, predictions_list):
        ...
        if self.loss[0] in ['log_loss', 'hinge_loss', 'accuracy']:
            ...
            aggregated_pred = stats.mode(
                predictions_array, axis=0)[0].flatten()
        else:
        ...
        if deep:
            for i, estimator in enumerate(self.best_estimator_):
                ...
                if hasattr(estimator, 'get_params'):
                    ...
                    params.update(
                        {f'model_{i}__{key}': value for key, value in estimator_params.items()})
        return params
    ...
    def shapley_additive_explanation(self, df_set_list, outname, dir_output, mode='bar', figsize=(50, 25), samples=None, samples_name=None):
        ...
        for i, estimator in enumerate(self.best_estimator_):
            ...
            sub_model.shapley_additive_explanation(
                df_set_list[i], f'{outname}_{i}', dir_output, mode, figsize, samples, samples_name)
            ...
            if key in ['models', 'loss', 'name']:
                ...
                if len(idx_and_param) == 1:
                    ...
                    if hasattr(self.best_estimator_[idx], 'set_params'):
                        self.best_estimator_[idx].set_params(
                            **{param_name: value})
            else:
...
class ModelStacking(Model):
    def __init__(self, models, final_estimator=None, loss='mse', name='ModelStacking'):
        ...
        self.name = name
        self.loss = loss
        self.is_classifier = self.loss in [
            'log_loss', 'hinge_loss', 'accuracy']
