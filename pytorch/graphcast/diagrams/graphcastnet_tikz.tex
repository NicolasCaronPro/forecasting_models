...
\begin{tikzpicture}[
    ...
    % Processor blocks
    \node[block, above right=1.4cm and 3.8cm of encoder, fill=green!15] (procenc) {Processor encoder\\GraphCastMeshProcessor\\1 layer (feed-forward)\\$\texttt{has\_time\_dim} = \textbf{False}$};
    ...
    \node[block, below right=1.4cm and 3.7cm of proccore, fill=green!9] (procdec) {Processor decoder\\GraphCastMeshProcessor\\1 layer\\$\texttt{has\_time\_dim} = \textbf{False}$};

    \draw[arrow] (encoder.north east) |- node[pos=0.3, above]{mesh edges \\ hidden $C_{\text{hidden}}$} (procenc.west);
    ...
    \draw[arrow] (procdec.south east) |- node[pos=0.3, above]{processed mesh nodes} (decoder.north);
    ...
    \node[below=2.6cm of decoder, align=left] (legend) {
        \begin{minipage}{15cm}
            ...
            \begin{itemize}
                ...
                \item Temporal gating disabled here: $\texttt{has\_time\_dim} = \textbf{False}$, so MeshProcessorBlock layers are used.
