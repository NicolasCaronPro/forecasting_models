...
def scale_data(time_series_data: np.ndarray) -> np.ndarray:
    ...
    reshaped_data = time_series_data.reshape(
    ...
    scaled_time_series_data = scaled_data.reshape(
        n_samples, n_timepoints, n_features)
...
def plot_pca_3D(distance_matrix, n_clusters: int, names: List[str] = None, save_dir=None) -> None:
    ...
    sc = ax.scatter(pca_result[:, 0], pca_result[:, 1],
                    pca_result[:, 2], c=cluster_labels, cmap='viridis', s=50)

    # Step 3: Annotate the points with hospital names
    for i, name in enumerate(names):
        ax.text(pca_result[i, 0], pca_result[i, 1],
                pca_result[i, 2], name, size=10, zorder=1, color='black')

    # Add labels and title
    ax.set_xlabel('PCA Component 1')
    ax.set_ylabel('PCA Component 2')
    ax.set_zlabel('PCA Component 3')
    title = '3D PCA Plot with Hospital Names'
    plt.title(title)
...
def plot_pca(distance_matrix, n_clusters: int, names: List[str] = None) -> None:
   # Step 2: Apply K-Means Clustering (or any other clustering algorithm)
    ...
    scatter = plt.scatter(
        dtw_pca[:, 0], dtw_pca[:, 1], c=cluster_labels, cmap='viridis', s=50)
    ...
    for i, name in enumerate(names):
        plt.text(dtw_pca[i, 0] + 0.02, dtw_pca[i, 1] + 0.02, name,
                 fontsize=9, ha='right')  # Adjust offset as needed
